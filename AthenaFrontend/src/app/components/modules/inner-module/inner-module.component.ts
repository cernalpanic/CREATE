...
export class InnerModuleComponent {
  ...
  public quests: Quest[] = [];
  protected allStudentQuestCompletion: StudentQuest[] = [];
  ...
  public async getAverageCompletionRate() {
    await this.getAllStudents();
    for (let i = 0; i < this.allStudents.length; i++) {
      const response = await this.studentService.GetModuleProgress(
        this.allStudents[i].roleID,
        false
      );
      this.studentModuleCompletionStatuses = response;
      for (let j = 0; j < this.studentModuleCompletionStatuses.length; j++) {
        if (
  ...
  async calculateTotalEarnedExp() {
    try {
      const studentQuests = await this.questService.GetStudentQuestCompletion(
        '00000000-0000-0000-0000-000000000000',
        this.module.ModuleID
      );
      if (studentQuests) {
        this.moduleStudentQuest = studentQuests.map(
          (studentQuest: any) => new StudentQuest(studentQuest)
        );
        const completedQuests = this.moduleStudentQuest.filter(
          (quest) => quest.Completed
        );
        console.log("All module's completed quests: ", completedQuests);
        this.totalEarnedExp = completedQuests.reduce(
          (acc, quest) => acc + quest.ExpGain,
          0
        );
      } else {
      ...
      console.error(
        'An error occurred while fetching quest completion for student',
        error
      );
    }
  }
  ...
  private async studentFilterQuests() {
    ...
    try {
      ...
      if (studentQuests) {
        this.allStudentQuestCompletion = studentQuests.map(
          (studentQuest: any) => new StudentQuest(studentQuest)
        );
      } else {
        console.error('Failed to fetch quest completion for student');
